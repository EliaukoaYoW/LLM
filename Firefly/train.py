...
def load_tokenizer(args):
    ...
    if 'internlm2' in args.model_name_or_path.lower():
        # internlm2 中下述特殊标记对应的 token_ids
        tokenizer._added_tokens_encoder.update({'<|im_start|>': 92543}) # 标记<|im_start|> 若存在 则其 ID 覆盖为 92543 不存在 则新增
...
def load_model(args, training_args):
    ...
    if args.train_mode == 'qlora':
        quantization_config = BitsAndBytesConfig(
            load_in_4bit=True,              # 是否使用 4-bit 量化加载模型
            bnb_4bit_compute_dtype=torch.float16 if training_args.fp16 else torch.bfloat16, # 设置计算使用的数据类型
            bnb_4bit_use_double_quant=True, # 是否使用双重量化（减少内存占用）
            bnb_4bit_quant_type="nf4",      # 4-bit 量化的类型 可选 fp4 或 nf4
            llm_int8_threshold=6.0,         # 用于激活 int8 推理的阈值
            llm_int8_has_fp16_weight=False, # 是否将权重保留为 fp16
        )
    else:
